...
class BasicLanguageModel:
    ...
    def get_data(self):
        with open("D:\\Statistics\\Final_Projects\\Blackcoffer\\articles\\Netclan20241017.txt",'r',encoding='utf-8') as file:
            corpus = file.read()
    ...
    def generate_text(self, context, num_tokens = 10):
        ...
        while len(generated) - len(context) < num_tokens:
            next_token = self.predict_next_tokens(generated[-(self.n_params-1):])
    ...
    def predict_next_token_with_temperature(self,context, temperature):
        for n in range(self.n_params,1,-1):
            if len(context) >= n-1:
                context_n  = tuple(context[-(n-1):])
                counts = self.state[n-1].get(context_n)
